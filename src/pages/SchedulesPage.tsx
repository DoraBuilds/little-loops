...
const input: CSSProperties = {
  width: '100%', boxSizing: 'border-box', border: '2px solid rgba(14,165,233,.18)',
  borderRadius: 16, padding: '14px 16px', fontFamily: font, fontSize: 20,
  fontWeight: 600, color: ink, background: '#fff',
};
const button = (background: string, color = '#fff'): CSSProperties => ({
  border: 'none', borderRadius: 16, padding: '14px 18px', background, color,
  fontFamily: font, fontSize: 18, fontWeight: 800, cursor: 'pointer',
});
...
interface DraggableTaskProps {
  ...
  taskTarget: ScheduleDay;
  onUpdateItems: (items: ScheduleItem[]) => void;
  onSetTaskTarget: (day: ScheduleDay) => void;
  onCopyTask: (entry: ScheduleItem) => void;
}
...
function DraggableTask({ entry, items, selectedDay, taskTarget, onUpdateItems, onSetTaskTarget, onCopyTask }: DraggableTaskProps) {
  ...
  const patch = (change: Partial<ScheduleItem>) =>
    onUpdateItems(items.map((item) => item.id === entry.id ? { ...item, ...change } : item));
  ...
  return (
    <Reorder.Item
      ...
      whileDrag={{ scale: 1.015, boxShadow: '0 18px 35px rgba(14,165,233,.2)', zIndex: 10 }}
      style={{ display: 'grid', gridTemplateColumns: '58px 130px 82px 1fr auto', gap: 12, alignItems: 'center', padding: 16, borderRadius: 22, background: '#f8fafc', position: 'relative' }}
    >
      <button
        type="button"
        aria-label={`Drag ${entry.title}`}
        onPointerDown={(event) => dragControls.start(event)}
        style={{ width: 52, height: 76, borderRadius: 16, border: '2px solid rgba(14,165,233,.14)', background: '#e0f2fe', color: '#0369a1', fontSize: 28, fontWeight: 900, cursor: 'grab', touchAction: 'none', fontFamily: font }}
        title="Drag to move this activity"
      >
        ≡
      </button>
      <input type="time" value={entry.time} onChange={(event) => patch({ time: event.target.value })} style={input} />
      <input value={entry.icon} onChange={(event) => patch({ icon: event.target.value })} style={{ ...input, textAlign: 'center', fontSize: 30, padding: '13px 8px' }} />
      <div>
        <input value={entry.title} onChange={(event) => patch({ title: event.target.value })} style={{ ...input, fontSize: 24, fontWeight: 800 }} />
        <input value={entry.note ?? ''} onChange={(event) => patch({ note: event.target.value })} placeholder="Optional note" style={{ ...input, marginTop: 8, fontSize: 18 }} />
        <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <select value={taskTarget} onChange={(event) => onSetTaskTarget(event.target.value as ScheduleDay)} style={{ ...input, width: 'auto', minWidth: 175, padding: '10px 12px', fontSize: 17 }}>
            {SCHEDULE_DAYS.filter((day) => day !== selectedDay).map((day) => <option key={day} value={day}>Copy task to {day}</option>)}
          </select>
          <button type="button" onClick={() => onCopyTask(entry)} style={{ ...button('#e0f2fe', '#0369a1'), padding: '10px 14px', fontSize: 17 }}>Copy task</button>
        </div>
      </div>
...
export default function SchedulesPage() {
  ...
  useEffect(() => {
    ...
    void scheduleRepo.load(household.id).then((loaded) => {
      ...
      if (cancelled) return;
      console.error('Could not load schedules', loadError); setError(errorMessage(loadError)); setStatus('error');
    });
  ...
  useEffect(() => {
    setCopyTarget(SCHEDULE_DAYS.find((day) => day !== selectedDay) ?? 'Monday');
    setTaskCopyTargets({});
  }, [selectedDay]);
  ...
  const reorderItems = (reordered: ScheduleItem[]) => {
    const originalTimes = (selected?.days[selectedDay] ?? []).map((item) => item.time).sort((a, b) => timeToMinutes(a) - timeToMinutes(b));
    updateItems(reordered.map((item, index) => ({ ...item, time: originalTimes[index] ?? item.time })));
  };

  const copyDay = () => {
    if (!selected || copyTarget === selectedDay) return;
    updatePlan({ days: { ...selected.days, [copyTarget]: (selected.days[selectedDay] ?? []).map((item) => ({ ...item, id: crypto.randomUUID() })) } });
  ...
  const copyTask = (entry: ScheduleItem) => {
    ...
    const targetItems = [...(selected.days[target] ?? []), { ...entry, id: crypto.randomUUID() }].sort((left, right) => timeToMinutes(left.time) - timeToMinutes(right.time));
  ...
  const save = async () => {
    ...
    const cleaned = cloneSchedules(schedules).map((schedule) => ({ ...schedule, days: Object.fromEntries(Object.entries(schedule.days).map(([day, items]) => [day, (items ?? []).slice().sort((left, right) => timeToMinutes(left.time) - timeToMinutes(right.time))])) }));
    try {
      await new SupabaseScheduleRepository(supabase).save(household.id, cleaned);
      setSchedules(cleaned); setStatus('saved'); window.setTimeout(() => setStatus('idle'), 1500);
    } catch (saveError) {
      console.error('Could not save schedules', saveError); setError(errorMessage(saveError)); setStatus('error');
    }
  };

  const createDisabled = status === 'loading' || status === 'error';
  const dayItems = selected?.days[selectedDay] ?? [];
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff9f0', fontFamily: font, color: ink, padding: '32px 20px' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        ...
        {status === 'loading' ? <div style={{ fontSize: 22 }}>Loading…</div> : status === 'error' ? (
          <div style={{ padding: 22, borderRadius: 18, background: '#fee2e2', color: '#b91c1c', fontSize: 20 }}>
            <strong>Could not load or save schedules.</strong><div style={{ marginTop: 8, fontSize: 18 }}>{error}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px,320px) minmax(0,1fr)', gap: 22 }}>
            ...
            <main>{!selected ? <div style={{ background: '#fff', borderRadius: 26, padding: 36, textAlign: 'center', color: mute, fontSize: 22 }}>Create or select a schedule.</div> : (
              <div style={{ background: '#fff', borderRadius: 26, padding: 24, border: '2px solid rgba(0,0,0,.07)' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 10 }}>
                  {children.map((child) => <label key={child.id} style={{ padding: '11px 15px', borderRadius: 99, background: selected.childIds.includes(child.id) ? '#e0f2fe' : '#f5ede2', cursor: 'pointer', fontSize: 19, fontWeight: 700 }}>
                    <input type="checkbox" checked={selected.childIds.includes(child.id)} onChange={(event) => updatePlan({ childIds: event.target.checked ? [...selected.childIds, child.id] : selected.childIds.filter((id) => id !== child.id) })} style={{ width: 20, height: 20, marginRight: 7 }} />{child.name}
                  </label>)}
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 8, marginTop: 24 }}>
                  {SCHEDULE_DAYS.map((day) => <button key={day} onClick={() => setSelectedDay(day)} style={{ ...button(day === selectedDay ? blue : '#f0f9ff', day === selectedDay ? '#fff' : '#0369a1'), padding: '13px 4px', fontSize: 17 }}>{day.slice(0, 3)}</button>)}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14, marginTop: 24, flexWrap: 'wrap' }}>
                  <div><h2 style={{ margin: 0, fontSize: 34 }}>{selectedDay}</h2><div style={{ color: mute, fontSize: 17, marginTop: 4 }}>Drag activities by the ≡ handle. Time slots stay in place.</div></div>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                    <select value={copyTarget} onChange={(event) => setCopyTarget(event.target.value as ScheduleDay)} style={{ ...input, width: 'auto', minWidth: 170 }}>{SCHEDULE_DAYS.filter((day) => day !== selectedDay).map((day) => <option key={day} value={day}>Copy to {day}</option>)}</select>
                    <button onClick={copyDay} style={button('#e0f2fe', '#0369a1')}>Copy day</button><button onClick={addItem} style={button(blue)}>+ Activity</button>
                  </div>
                </div>
                <Reorder.Group axis="y" values={dayItems} onReorder={reorderItems} style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 16, padding: 0, listStyle: 'none' }}>
                  {dayItems.map((entry) => <DraggableTask key={entry.id} entry={entry} items={dayItems} selectedDay={selectedDay} taskTarget={taskCopyTargets[entry.id] ?? SCHEDULE_DAYS.find((day) => day !== selectedDay) ?? 'Monday'} onUpdateItems={updateItems} onSetTaskTarget={(day) => setTaskCopyTargets((previous) => ({ ...previous, [entry.id]: day }))} onCopyTask={copyTask} />)}
                </Reorder.Group>
